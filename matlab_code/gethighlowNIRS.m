function [BRT1,highRT,lowRT,entireRT]=gethighlowNIRS(car1,car2,entireRT)
...
for s=1:length(brakelabel2)
    for t=1:length(brakelabel1)
        if brakelabel1(t)-brakelabel2(s)<3.0 && 0.3<brakelabel1(t)-brakelabel2(s)
...
entireRT=vertcat(entireRT,vertcat(length(lowRT),length(highRT)));
end
